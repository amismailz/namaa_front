...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pageKey = `/${ROUTES.SOCIAL_MEDIA}` // <-- replace with current page route // e.g., 'contact-us', 'about-us', etc.
  ...
  return {
    title: t("seo.social_media.title"),
    description: t("seo.social_media.description"),
    alternates: {
      canonical: url,
      languages: {
...
export default function SocialMediaPage() {
  return (
    <>
      <HeroPage
        heading={<Translate id="navbar.social_media" />}
        breadcrumb={[
        ...
        <Container className="space-y-10">
          <header>
            <h3 className="font-medium text-3xl">
              <Translate id="social_media.title" />
            </h3>
          </header>

          <div className="font-light text-lg space-y-7">
            <p>
              <Translate id="social_media.p1" />
            </p>
          </div>

          <div className="flex items-center justify-center gap-3">
            <ServiceRequestDialog />
            <ServiceWhatsAppButton currentUrl={`/${ROUTES.SOCIAL_MEDIA}`} />
