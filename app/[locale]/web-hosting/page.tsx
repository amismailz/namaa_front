...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pageKey = `/${ROUTES.WEB_HOSTING}` // <-- replace with current page route // e.g., 'contact-us', 'about-us', etc.
  ...
  return {
    title: t("seo.web_hosting.title"),
    description: t("seo.web_hosting.description"),
    alternates: {
      canonical: url,
      languages: {
...
export default function WebHostingPage() {
  return (
    <>
      <HeroPage
        heading={<Translate id="navbar.web_host" />}
        breadcrumb={[
        ...
        <Container className="space-y-10">
          <header>
            <h3 className="font-medium text-3xl">
              <Translate id="web_hosting.title" />
            </h3>
          </header>

          <div className="font-light text-lg space-y-7">
            <p>
              <Translate id="web_hosting.p1" />
            </p>
          </div>

          <div className="flex items-center justify-center gap-3">
            <ServiceRequestDialog />
            <ServiceWhatsAppButton currentUrl={`/${ROUTES.WEB_HOSTING}`} />
